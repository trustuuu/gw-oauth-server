...
export function authenticate(req: Request, res: Response, next: NextFunction) {
  ...
  if (payload.iss === process.env.UNIDIR_ISSUER) {
    jwtCheck(req, res, (err: any) => {
      ...
      (req as any).authService = (req as any).auth;
      next();
    });
  } else if (payload.iss === process.env.UNIDIR_SERVICE_ISSUER) {
    jwtCheckService(req, res, (err: any) => {
      if (err) {
 ...
 */
export async function getVerifyJwtWithJwks(token: string) {
  const header: any = decodeHeader(token);
...
export function checkTenantAccess(
  ...
  const tokenTenant = (req as any).auth?.tenant_id;
