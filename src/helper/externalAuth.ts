 ...
 */
export async function getGoogleInitialTokens(
  connection: Connection,
...
export async function refreshGoogleAccessToken(
  ...
  domainId: string,
  userId: string,
  account: any, // ExternalIdentityAccount type?
  connection: Connection
) {
...
export async function getAzureAccessToken(refreshToken: string) {
  const params = new URLSearchParams({
  ...
  const data: any = await (response as any).data; // Original code said 'response.data', but fetch returns Response object which has .json(). 'axios' returns .data. Using fetch here, so probably .json(). BUT original code said 'response.data'. If original used axios, then response.data is correct.
  // Wait, original code:
