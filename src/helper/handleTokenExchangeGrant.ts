...
const rsaKeyObj = KEYUTIL.getKey(rsaKey as any);
const PRIVATE_KEY_PEM = KEYUTIL.getPEM(rsaKeyObj, 'PKCS8PRV');

const rsaKeyServiceObj = KEYUTIL.getKey(rsaKeyService as any);
const PRIVATE_KEY_PEM_SERVICE = KEYUTIL.getPEM(rsaKeyServiceObj, 'PKCS8PRV');

function fetchJwks(url: string): Promise<Record<string, string>> {
    ...
    https.get(url, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          const keys: Record<string, string> = jwks.keys.reduce((acc: any, jwk: any) => {
...
function verifyJwt(token: string, publicKeys: Record<string, string>) {
  const [headerB64, payloadB64] = token.split('.');
  const header: any = KJUR.jws.JWS.readSafeJSONString(b64utoutf8(headerB64));
...
function signJwt(payload: any, privateKey: any) {
  ...
  return KJUR.jws.JWS.sign(
    ...
    privateKey // This is the PEM string
  );
}
...
export async function handleTokenExchange(req: Request, res: Response) {
  ...
  const allowedScopes = requestedScopes.filter((s: string) => subjectPayload.permissions?.includes(s));
