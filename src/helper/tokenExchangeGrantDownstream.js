 ...
 * - subject_token
 * - subject_token_type (expect access_token)
 * - actor_token (optional)
 * - actor_token_type (optional)
 * - audience / resource (target audience)
 * - scope (optional – requested scopes)
 * - requested_token_type (optional – we’ll always issue access_token)
 */
export const tokenExchangeGrantDownstream = async (
  ...
  try {
    ...
    // Optional: if you allow external IdP tokens, this is where you'd:
    // - Detect issuer (subjectPayload.iss)
    // - Use connection API / ExternalIdentityAccount to validate externally
    // TODO: integrate ExternalIdentityAccount / connection API here if needed.
    ...
    const lifetimeByPolicy = maxTokenLifetimeSeconds || 3600;
    const expiresInSeconds = Math.min(remainingFromSubject, lifetimeByPolicy);
