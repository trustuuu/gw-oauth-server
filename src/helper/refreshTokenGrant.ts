...
export const refreshTokenGrant = async (req: Request, res: Response) => {
  const { companyId, domainId, userId, sessionId } = JSON.parse(
    req.cookies.user || "{}"
  );

  let deviceId = req.headers[process.env.DEVICE_ID_HEADER || ""] as string;
  const {
    client_id: clientId,
  ...
  const refreshTokenBodyPlain: any = await getVerifyJwtWithJwks(
    refreshTokenBody
  );
  ...
  if (refreshTokenServer) {
    ...
    if (refreshTokenServer.scope.includes("openId")) {
      const id_token = generateIdToken(
        ...
        clientId,
        refreshTokenServer.scope,
        "nonce" // Missing nonce, adding placeholder
      );
