...
export function ntlmV1HashHex(password: string) {
  const bytes = utf16leBytes(password);
  return md4Hex(bytes);
}

export function convertPassword(body: any) {
...
function utf16leBytes(str: string) {
...
function md4Hex(bytes: Uint8Array) {
  const digest = md4(bytes);
  return [...digest].map((b) => b.toString(16).padStart(2, "0")).join("");
}

// ---- Minimal MD4 (RFC 1320) ----
function md4(messageBytes: Uint8Array) {
