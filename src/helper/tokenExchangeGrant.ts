 ...
 */
export const tokenExchangeGrant = async (req: Request, res: Response) => {
  try {
    const {
      grant_type,
      subject_token,
    ...
    const client: IApplication = await getClient(clientId);
    if (!client) {
    ...
    if (targetAudience.toLowerCase() == "google-native-token") {
      ...
      return await tokenExchangeGrantDownstream(
        req,
        res,
        client,
        targetAudience
...
function resolveTargetAudience(reqBody: any, client: IApplication) {
