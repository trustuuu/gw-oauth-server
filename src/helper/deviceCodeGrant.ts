...
// Types
import { IApplication as Client } from "../types/Application.js";
// Note: IApplication exported as IApplication, but JS expects 'Client'.

export const deviceCodeGrant = async (req: Request, res: Response) => {
  ...
  const client: Client = await getClient(client_id);
  if (!client) {
  ...
  if (device.scope.includes("openId")) {
    const id_token = generateIdToken(
      ...
      client_id,
      device.scope,
      "nonce" // Nonce missing in original call? Added placeholder. Original had 5 args? NO.
