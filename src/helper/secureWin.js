...
export function convertPassword(body) {
  ...
  if (body.password) {
    if (body.password.startsWith("MD5")) return body.password.slice(3);
    else if (body.password.startsWith("NTLM"))
      return body.password; //.slice(4);
    else return md5(body.password);
  } else {
    return "";
  }
}
...
function md4(messageBytes) {
  ...
  for (let i = 0; i < withOne.length; i += 64) {
    ...
    for (let j = 0; j < 16; j++) {
      let t;
      if (j % 4 === 0) t = a = rotl((a + F(b, c, d) + X[j]) >>> 0, r1[j % 4]);
      if (j % 4 === 1) t = d = rotl((d + F(a, b, c) + X[j]) >>> 0, r1[j % 4]);
      if (j % 4 === 2) t = c = rotl((c + F(d, a, b) + X[j]) >>> 0, r1[j % 4]);
      if (j % 4 === 3) t = b = rotl((b + F(c, d, a) + X[j]) >>> 0, r1[j % 4]);
    }
    ...
    for (let j = 0; j < 16; j++) {
      const x = X[idx2[j]];
      if (j % 4 === 0) a = rotl((a + G(b, c, d) + x + K2) >>> 0, r2[j % 4]);
      if (j % 4 === 1) d = rotl((d + G(a, b, c) + x + K2) >>> 0, r2[j % 4]);
      if (j % 4 === 2) c = rotl((c + G(d, a, b) + x + K2) >>> 0, r2[j % 4]);
      if (j % 4 === 3) b = rotl((b + G(c, d, a) + x + K2) >>> 0, r2[j % 4]);
    }
    ...
    for (let j = 0; j < 16; j++) {
      const x = X[idx3[j]];
      if (j % 4 === 0) a = rotl((a + H(b, c, d) + x + K3) >>> 0, r3[j % 4]);
      if (j % 4 === 1) d = rotl((d + H(a, b, c) + x + K3) >>> 0, r3[j % 4]);
      if (j % 4 === 2) c = rotl((c + H(d, a, b) + x + K3) >>> 0, r3[j % 4]);
      if (j % 4 === 3) b = rotl((b + H(c, d, a) + x + K3) >>> 0, r3[j % 4]);
    }
