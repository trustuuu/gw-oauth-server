...
routerApplication.get(`/${appPath}/`, guard, (req: any, res) => {
...
routerApplication.post(`/${appPath}/`, guard, (req, res) => {
  ...
  httpRun(
    res,
    () => applicationService.setData(data, appPath, data.client_id),
    undefined,
    undefined,
    data
  );
...
routerApplication.post(
  `/${appPath}/:id/${PermissionScopes}`,
  guard,
  createPostHandler(applicationService as any, (req: any) => [
    appPath,
    req.params.id,
    PermissionScopes,
  ])
...
routerApplication.post(
  ...
  createPostHandler(
    ...
    (req: any) => [appPath, req.params.id, TokenExchanges],
    {
      generateIdFn: (item: any) => generateId(item.name),
    }
  )
);
...
routerApplication.put(`/${appPath}`, guard, (req, res) => {
  ...
  httpRun(
    res,
    () => applicationService.updateData(data, appPath, data.id),
    null as any,
    null as any,
    data
  );
...
routerApplication.put(`/${appPath}/:id`, guard, (req, res) => {
  ...
  httpRun(
    res,
    () => applicationService.updateData(data, appPath, req.params.id),
    null as any,
    null as any,
    data
  );
...
routerApplication.delete(`/${appPath}/:id`, guard, (req, res) => {
  const data = {};
  httpRun(res, () =>
    applicationService.deleteData(data, appPath, req.params.id)
  );
});
...
routerApplication.delete(`/${appPath}`, guard, (req, res) => {
  ...
  const allDeletes = data.map((item) => {
    return applicationService.deleteData(null as any, appPath, item.id);
  });
...
routerApplication.delete(
  ...
  (req, res) => {
    const data = [...req.body];
    const allDeletes = data.map((item) => {
      return applicationService.deleteData(
        null as any,
        appPath,
        req.params.id,
        PermissionScopes,
