...
routerDomain.get(`/:id/${DOMAIN_COLL}/primary`, (req, res) => {
    console.log('get primary', req.params);
...
routerDomain.post(`/:id/${DOMAIN_COLL}`, async (req, res) => {
    const data = {...req.body, id:generateId(req.body.name), whenCreated: new Date(), status: "New"};
    const domain = await domainService.getDomainInfo(req.params.id, data.id);
    if (domain.name)
    {
        return res.status(409).send('Item exists');
    }
    run(res, () => domainService.setData.apply(domainService, [data].concat([req.params.id, data.id])));
});

routerDomain.put(`/:id/${DOMAIN_COLL}/:domainId`, (req, res) => {
    const data = {...req.body, whenUpdated: new Date(), status: "Updated"};
    run(res, () => domainService.updateData.apply(domainService, [data].concat([req.params.id, req.params.domainId])));
});

routerDomain.put(`/:id/${DOMAIN_COLL}`, (req, res) => {
    const data = req.body;
    const allDomains = data.map(item => {
        return domainService.updateData.apply(domainService, [{...item, whenUpdated: new Date(), status: "Updated"}].concat([req.params.id, item.id]))
    });
...
routerDomain.delete(`/:id/${DOMAIN_COLL}/:domainId`, (req, res) => {
    const data = {};
    run(res, () => domainService.deleteData.apply(domainService, [data].concat([req.params.id, req.params.domainId])));
});

routerDomain.delete(`/:id/${DOMAIN_COLL}`, (req, res) => {
    const data = [...req.body];
    const allDeletes = data.map(item => {
        return domainService.deleteData.apply(domainService, [null].concat([req.params.id, item.id]))
    });
...
{
  return fn()
    .then(result => response.status(200).send(success ? success(result) : result))
    .catch(err => {
        console.error(err);
