...
export async function device_authorization(
  req: Request,
  res: Response,
  routerAuth: any
) {
  const { client_id, scope } = req.body;
  const client: any = await getClient(client_id);
  if (!client) {
  ...
  const user_code = generateUserCode(
    (process.env.USER_CODE_FORMAT as string).replace(/[A-Z0-9]/g, "A")
  );
  const data = {
  ...
  await deviceService.setData(data, AUTH_PATH, device_code);
