...
export async function logout(req: Request, res: Response, routerAuth: any) {
  const { redirect_url } = req.query;
  const deviceId = req.headers[process.env.DEVICE_ID_HEADER as string];
  if (req.cookies.user) {
    ...
    const user: any = await userService.getData(companyId, domainId, userId);
    ...
    const refreshTokenServer: any = await refreshTokenService.getData(
      AUTH_PATH,
      deviceId as any
    );
    ...
    await refreshTokenService.deleteData({}, AUTH_PATH, deviceId as any);
