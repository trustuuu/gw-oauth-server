...
async function redirectToLogin(reqQuery: any, res: Response) {
  const params = {
...
const allowUserConsentSkip = async (res: Response, client: any) => {
  const api: any = await apiService.getApiByAudience(API_PATH, client.audience);
  if (api.length < 1) {
    console.log(
