...
async function approve(req: Request, res: Response, routerAuth: any) {
  ...
  const query: any = await reqIdService.getData(AUTH_PATH, reqid);
  ...
  if (req.body.approve) {
    if (query.response_type == "code") {
      ...
      if (Array.isArray(clientScopes)) {
        clientScopes = clientScopes.map((s: any) => s.permission);
      }
