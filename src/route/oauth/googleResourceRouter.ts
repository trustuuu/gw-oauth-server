...
import connectionService from "../../service/connection-service.js"; // corrected path

const externalIdentityAccountService: any = {}; // Placeholder for missing service
 ...
 */
async function refreshGoogleAccessToken(account: any, connection: any) {
 ...
 */
async function ensureGoogleAccessToken(extAccount: any, connection: any) {
 ...
 */
async function findUserGoogleAccount({ companyId, domainId, userId }: any) {
 ...
 */
async function findGoogleConnection({ companyId, domainId }: any) {
  // Assume connectionService.getGoogleConnection exists or use generic get
  const connection = await (connectionService as any).getGoogleConnection({
    companyId,
    domainId,
    provider: GOOGLE_PROVIDER,
  });
 ...
 */
router.get("/google/userinfo", authenticate, async (req: any, res) => {
  try {
    ...
    if (!googleResp.ok) {
      ...
      console.error("Google userinfo call failed:", googleResp.status, body);
 ...
 */
router.all("/google/proxy/*", authenticate, async (req: any, res) => {
  try {
    const auth = req.authService || req.auth;
    ...
    Object.entries(req.query || {}).forEach(([k, v]: any) => {
      if (Array.isArray(v)) {
        v.forEach((vv: any) => googleUrl.searchParams.append(k, vv));
      } else {
