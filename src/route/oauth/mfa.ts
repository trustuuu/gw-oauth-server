...
export const mfaGet = async (req: Request, res: Response, routerAuth: any) => {
  const txId = req.query.tx as string;
  const tx: any = await reqidService.getData(AUTH_PATH, txId);
...
export const mfaPost = async (req: Request, res: Response, routerAuth: any) => {
  const tx: any = await reqidService.getData(AUTH_PATH, req.body.tx);
  ...
  await reqidService.deleteData({}, AUTH_PATH, req.body.code);
  ...
  if (txId) {
    ...
    const authorizationEndpoint =
      process.env.NODE_ENV === "dev"
        ? (routerAuth as any).locals.authorizationEndpointDev
        : (routerAuth as any).locals.authorizationEndpoint;
...
const verifyTotp = async (secretBase32: string, code: string) => {
  const ok = verifyMfaToken(secretBase32, code);
  return ok;
};

export const getDecryptMfaLink = async (
  req: Request,
  res: Response,
  routerAuth: any
) => {
