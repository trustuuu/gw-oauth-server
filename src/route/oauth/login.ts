...
async function login(req: Request, res: Response, routerAuth: any) {
  let authorizequery: any = {
    response_type: "code",
  ...
  let userVerified: any = { user: null, verified: false };

  if (R.includes("openId", (reqQuery.scope as string).split(" "))) {
    if (!reqQuery.email || !reqQuery.password) {
      redirectToLogin(reqQuery, res);
      return;
    }

    let user: any = await getUserRef(reqQuery.email);
    if (!user) {
    ...
    if (user) {
      ...
      if (!userVerified.verified) {
        ...
        if (mfaRequired) {
          ...
          await reqidService.setData(tx, AUTH_PATH, txId);
