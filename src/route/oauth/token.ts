...
async function token(req: Request, res: Response, routerAuth: any) {
  if (
  ...
  let clientScopes: any =
    await applicationService.getApplicationPermissionScopes(APP_PATH, clientId);
  ...
  if (req.body.grant_type == "client_credentials") {
    ...
    const api: any = await apiService.getApiByAudience(
      API_PATH,
      client.audience
    );
    if (api.length < 1) {
