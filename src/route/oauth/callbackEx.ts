...
export const callbackEx = async (
  ...
  if (state) {
    try {
      ...
      const decodedStateJson = Buffer.from(state as string, "base64").toString(
        "utf8"
      );
  ...
  try {
    ...
    const token_data = await getGoogleInitialTokens(connection, code as string);
    //
