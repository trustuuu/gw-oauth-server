import userService from "../../service/user-service.js";
import companyService from "../../service/company-service.js";
import applicationService from "../../service/application-service.js";
import authService from "../../service/authorization-service.js";
import md5 from "blueimp-md5";
import { AUTH_PATH, generateId } from "../../service/remote-path-service.js";
import domainService from "../../service/domain-service.js";
import crypto from "crypto";

const appId = "application";

export const authData = async () => {
  return await authService.getData(AUTH_PATH);
};

export const getUserRef = async (email: string) => {
  const user: any = await userService.getUserRef(email);
  if (user && user.session) {
    delete user.session;
  }
  return user;
};

export const createAuthUser = async (
  companyId: string,
  domainId: string,
  userId: string
) => {
  await userService.createAuthUser(companyId, domainId, userId);
};

export const getUser = async (
  companyId: string,
  domainName: string,
  email: string
) => {
  const users: any = await userService.getUserByEmail(
    companyId,
    domainName,
    email
  );
  let user = null;
  if (users.length == 1) {
    user = users[0];
    if (user.session) delete user.session;
  }

  return user;
};

export const getUserAppRoles = async (
  companyId: string,
  domainName: string,
  email: string,
  condition: any
) => {
  const users: any = await userService.getUserAppRoles(
    companyId,
    domainName,
    email,
    condition
  );
  let user = null;
  if (users.length == 1) {
    user = users[0];
  }

  return user;
};

export const getUserById = async (
  companyId: string,
  domainName: string,
  userId: string
) => {
  const users: any = await userService.getUsersWhere(companyId, domainName, [
    "id",
    "==",
    userId,
  ]);
  let user = null;
  if (users.length == 1) {
    user = users[0];
  }

  return user;
};

export const verifyUser = async (
  companyId: string,
  domainName: string,
  email: string,
  password: string
) => {
  const userVerified = await userService.userVerification(
    companyId,
    domainName,
    email,
    password
  );

  return userVerified;
};

export const getClient = async (clientId: string) => {
  let appData: any = await applicationService.getData(appId, clientId);
  let appPermissionData: any =
    await applicationService.getApplicationPermissionScopes(appId, clientId);

  appData.PermissionScopes = appPermissionData
    ? Array.isArray(appPermissionData)
      ? appPermissionData.map((p: any) => p.permission)
      : []
    : [];
  if (appData.client_id == undefined) return null;

  return appData;
};

export const saveTokenToDB = async (data: any) => {
  const {
    companyId,
    domainId,
    accessToken,
    userId,
    sessionId,
    refreshToken,
    idToken,
  } = data;

  await userService.updateData(
    {
      session: {
        sessionId,
        accessToken,
        refreshToken,
        idToken,
        whenUpdated: new Date(),
      },
    },
    companyId,
    domainId,
    userId
  );
};

export const SignUpCompany = async (comapnyData: any) => {
  const newName = comapnyData.name.replace(/\s+/g, "");
  const data = {
    ...comapnyData,
    name: newName,
    id: `${newName}-${crypto.randomUUID()}`,
    whenCreated: new Date(),
    status: "New",
  };
  const company: any = await companyService.getCompanyByName(data.name);
  if (company.length > 0) {
    throw new Error("company exists");
  }
  await companyService.setData(data, data.id);
  return data;
};

export const SignUpDomain = async (companyId: string, domainData: any) => {
  const data = {
    ...domainData,
    id: domainData.name,
    whenCreated: new Date(),
    status: "New",
  };
  const domain: any = await domainService.getDomainByName(companyId, data.name);
  if (domain.length > 0) {
    throw Error("Domain exists");
  }
  await domainService.setData(data, companyId, data.id);
  return data;
};
export const SignUpUser = async (
  companyId: string,
  domainId: string,
  userData: any
) => {
  const data = {
    ...userData,
    id: generateId(userData.userName),
    authVerification: md5(userData.authVerification),
    whenCreated: new Date(),
    status: "new",
  };

  const user: any = await userService.getUserByEmail(
    companyId,
    domainId,
    data.email
  );
  if (user.length > 0) {
    console.log("User already existed", user);
    throw Error("User already existed");
  }
  await userService.setData(data, companyId, domainId, data.id);
  if ("root" in data) {
    await createAuthUser(companyId, domainId, data.id);
  }

  return data;
};
