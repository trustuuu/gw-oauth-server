...
export const Guard = {
    check: function (requiredScopes) {
        return function (req, res, next) {
            const auth = req.auth;
            const permissions = auth && Array.isArray(auth.permissions) ? auth.permissions : [];
...
export const GuardLeast = {
    check: function (requiredScopes, requiredRoles) {
        return async function (req, res, next) {
            ...
            console.log("requiredScopes, requiredRoles", requiredScopes, requiredRoles);
            //Tenant Checking
            ...
            if (req.originalUrl.endsWith("/companys") && req.method === "POST") {
                console.log(`Tenant checking has been passed for ${req.originalUrl} : Method: ${req.method}`);
            }
                ...
                else if (req.originalUrl.endsWith("/companys") &&
                    tokenTenant &&
                    req.method === "DELETE" &&
                    req.body) {
                    console.log("trying company_all by deleteTenant", req.originalUrl, req.body[0]);
            ...
            const scopesAuthorized = requiredScopes === undefined
                ? requiredScopes
                : !requiredScopes
                    ? true
                    : requiredScopes.some(function (scopeGroup) {
                        if (Array.isArray(scopeGroup)) {
                            ...
                            return scopeGroup.every(function (scope) {
                                console.log(`permissions => ${permissions} includes scope => ${scope}`);
                                return permissions.includes(scope);
                            });
                        }
                        else {
                            // OR Mode: one of permissions included in scope.
                            console.log(`permissions => ${permissions} includes scopeGroup => ${scopeGroup}`);
                            return permissions.includes(scopeGroup);
                        }
                    });
            // Role OR checking
            //const rolesAuthorized = !requiredRoles
            const rolesAuthorized = requiredRoles === undefined
                ? requiredRoles
                : !requiredRoles
                    ? true
                    : requiredRoles.some(function (roleGroup) {
                        if (Array.isArray(roleGroup)) {
                            ...
                            console.log(`roleGroup => ${roleGroup} includes roles => ${roles}`);
                            return roleGroup.every(function (role) {
                                return roles.includes(role);
                            });
                        }
            ...
            console.log("scopesAuthorized => ", scopesAuthorized, "rolesAuthorized => ", rolesAuthorized);
            if ((scopesAuthorized === true || rolesAuthorized === true) &&
