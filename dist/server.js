...
try {
    ...
    const fetchOriginFromDB = async (origin, req) => {
        try {
            let clientId = "";
            const auth = req.headers["authorization"];
            if (auth) {
            ...
            if (client.allowed_web_orgins.some((o) => o === origin)) {
                console.log(client.client_id, ": allowed_web_orgins set to ", client.allowed_web_orgins);
    ...
    app.use(async (req, res, next) => {
        try {
            ...
            const isAuthRoute = ["/oauth/v1/token", "/oauth/v1/signup", "/oauth/v1/forget-pw", "/oauth/v1/forgot-pw-reset"].includes(cleanPath);
            // 3. Check DB for the origin
            const isAllowed = await fetchOriginFromDB(cleanOrigin, req);
            console.log(`Origin: ${cleanOrigin} | Path: ${cleanPath} | Allowed: ${isAllowed}`);
            ...
            res.setHeader("Access-Control-Allow-Headers", `Origin, X-Requested-With, Content-Type, Accept, Authorization, ${process.env.DEVICE_ID_HEADER || "x-device-id"}`);
    ...
    app.use(`/oauthapi/v1`, cors(corsOptions), authenticate, routerApplication, routerApi);
    app.post(`/v1/${COMPANY_COLL}/:companyId/domainNames/:domainId/purge/cors`, cors(corsOptions), authenticate, GuardLeast.check(undefined, [
        ["tenant:admin"],
        ["company:admin"],
        ["Ops:Admin"],
    ]), (req, res) => {
    ...
    app.use(`/v1/${COMPANY_COLL}`, cors(corsOptions), authenticate, routerCompany, routerDomain, routerGroup, routerUser, routerConnection, routerProvisioning, routerScim);
}
catch (error) {
...
app.options("/jwks.json", (req, res) => {
    ...
    res.setHeader("Access-Control-Allow-Headers", `Content-Type, Authorization, ${process.env.DEVICE_ID_HEADER}`);
...
const server = app.listen(port || 8080, hostheader, () => {
    ...
    console.log("OAuth Authorization Server[%s] is listening at http://%s:%s", isProduction ? "PRODUCTION" : "DEV", host, port);
});
