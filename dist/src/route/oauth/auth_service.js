...
export const getUserAppRoles = async (companyId, domainName, email, condition) => {
    const users = await userService.getUserAppRoles(companyId, domainName, email, condition);
    let user = null;
    if (users.length == 1) {
        user = users[0];
    }
...
export const verifyUser = async (companyId, domainName, email, password) => {
    const userVerified = await userService.userVerification(companyId, domainName, email, password);
    return userVerified;
};
export const getClient = async (clientId) => {
    let appData = await applicationService.getData(appId, clientId);
    let appPermissionData = await applicationService.getApplicationPermissionScopes(appId, clientId);
    appData.PermissionScopes = appPermissionData
        ? Array.isArray(appPermissionData)
            ? appPermissionData.map((p) => p.permission)
            : []
        : [];
    if (appData.client_id == undefined)
        return null;
    return appData;
};
export const saveTokenToDB = async (data) => {
    const { companyId, domainId, accessToken, userId, sessionId, refreshToken, idToken, } = data;
...
export const SignUpUser = async (companyId, domainId, userData) => {
    ...
    const user = await userService.getUserByEmail(companyId, domainId, data.email);
    if (user.length > 0) {
