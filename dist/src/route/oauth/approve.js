...
async function approve(req, res, routerAuth) {
    ...
    //delete routerAuth.locals.requests[reqid]; // Locals are not persistent or used here as per TS migration where we use DB
    // Original code deleted it from locals but then retrieved from DB.
    const query = await reqIdService.getData(AUTH_PATH, reqid);
    await reqIdService.deleteData({}, AUTH_PATH, reqid);
    if (!query) {
    ...
    if (req.body.approve) {
        if (query.response_type == "code") {
            ...
            //   req.body.email
            // );
            let clientScopes = await applicationService.getApplicationPermissionScopes(APP_PATH, query.client_id);
            ...
            const urlParsed = await generateCodeUrlBuild({ ...query, password: null }, req.body.email, rscope, req.body.code_challenge, req.body.code_challenge_method);
            res.redirect(urlParsed);
            return;
        }
        else {
