...
import { API_PATH, APP_PATH, AUTH_PATH, } from "../../service/remote-path-service.js";
...
async function authorize(req, res, routerAuth) {
    ...
    // Warning: client might be null here but code usage suggests checking below.
    // Actually line 44 check !client.
    // BUT R.includes below usage will crash if client is null.
    // I should move client check up or ensure client is not null before checking grant_types.
    if (client) {
        if (R.includes("authorization_code", client.grant_types)) {
            if (!reqQuery.email && !reqQuery.password && reqQuery.stage != "mfa") {
                // Note: In TS, function calls that end response flow don't automatically stop execution unless return.
    ...
    if (!client) {
        ...
        console.log("Mismatched redirect URI, expected %s got %s", client.redirect_uris, reqQuery.redirect_uri);
        ...
        rscope = reqQuery.scope
            ...
            : undefined;
        let clientScopes = await applicationService.getApplicationPermissionScopes(APP_PATH, reqQuery.client_id);
    ...
    if (userConsent) {
        const redirectURL = await generateCodeUrlBuild({ ...reqQuery, password: null }, reqQuery.email, reqQuery.scope.split(" "), reqQuery.code_challenge, reqQuery.code_challenge_method);
        res.redirect(redirectURL);
        return;
    }
    else {
...
const allowUserConsentSkip = async (res, client) => {
    ...
    if (api.length < 1) {
        console.log("Authence has not been found, expected %s got %s", client.Id, client.audience);
