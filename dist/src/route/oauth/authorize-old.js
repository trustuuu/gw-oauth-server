...
async function authorize(req, res, routerAuth) {
    ...
    let userVerified = { verified: false };
    ...
    if (R.includes("openId", (reqQuery.scope || "").split(" "))) {
        if (!reqQuery.email && !reqQuery.password) {
            ...
            if (user) {
                userVerified = await verifyUser(user.companyId, user.domainId, reqQuery.email, reqQuery.password);
            }
...
const allowUserConsentSkip = async (res, client) => {
    const api = await apiService.getApiByIdentifier(API_PATH, client.audience);
    if (api.length < 1) {
