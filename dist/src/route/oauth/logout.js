...
export async function logout(req, res, routerAuth) {
    ...
    if (req.cookies.user) {
        ...
        const refreshTokenBodyPlain = await getVerifyJwtWithJwks(user.session.refreshToken);
        ...
        if (refreshTokenServer.refresh_token != refreshTokenBodyPlain.jti) {
            console.log(`refresh_token_mismatch ${refreshTokenServer.refresh_token}:${refreshTokenBodyPlain.jti}`);
        ...
        if (!refreshTokenServer ||
            ...
            console.log(`session_information_mismatch ${companyId}:${refreshTokenServer.user.companyId}=>${domainId}:${refreshTokenServer.user.domainId}`);
            return res.status(401).json({ error: "session_information_mismatch" });
        }
        await refreshTokenService.deleteData({}, AUTH_PATH, deviceId);
