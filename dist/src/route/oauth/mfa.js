...
import { buildUrl, decryptPayload, encryptPayload, toBase32, } from "../../helper/utils.js";
...
export const mfaGet = async (req, res, routerAuth) => {
    ...
    if (!tx || tx.stage !== "mfa")
        return res.status(400).send("Invalid tx");
...
export const mfaPost = async (req, res, routerAuth) => {
    ...
    if (!tx || tx.stage !== "mfa")
        return res.status(400).send("Invalid tx");
    const { totp } = req.body;
    const ok = await verifyTotp(`${tx.userId}${process.env.TOTP_SECRET}`
        .replace(/[^a-zA-Z0-9]/g, "")
        .toUpperCase(), totp); // your TOTP/WebAuthn verification
    if (!ok)
...
export const mfaImage = async (req, res, routerAuth) => {
    ...
    const codeImage = await getQRCodeImageUrl(d, e, 
    //process.env.TOTP_SECRET
    toBase32(`${u}${process.env.TOTP_SECRET}`
        .replace(/[^a-zA-Z0-9]/g, "")
        .toUpperCase()));
...
export const mfaVerify = async (req, res, routerAuth) => {
    ...
    if (txId) {
        const tx = await reqidService.getData(AUTH_PATH, txId);
        const authorizationEndpoint = process.env.NODE_ENV === "dev"
        ...
        //process.env.TOTP_SECRET,
        toBase32(`${user}${process.env.TOTP_SECRET}`
            .replace(/[^a-zA-Z0-9]/g, "")
            .toUpperCase()), totp);
        if (!ok)
            return res.status(401).send("Invalid code");
