...
import { decodeClientCredentials, generateServiceAccessToken, getUTCNow,
//getJwtExpire,
...
async function token(req, res, routerAuth) {
    ...
    if (req.body.client_id) {
        if (clientId) {
            ...
            console.log("Client attempted to authenticate with multiple methods", routerAuth);
    ...
    let clientScopes = await applicationService.getApplicationPermissionScopes(APP_PATH, clientId);
    ...
    if (!R.includes(req.body.grant_type, client.grant_types)) {
        console.log("Mismatched grant_type %s does not exist in %s", req.body.grant_type, client.grant_types.join(","));
    ...
    else if (req.body.grant_type == "urn:ietf:params:oauth:grant-type:device_code") {
