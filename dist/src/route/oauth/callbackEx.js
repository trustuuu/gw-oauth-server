import { findConnection, findExternalAccount, getGoogleInitialTokens, updateTokenToServer, } from "../../helper/externalAuth.js";
...
export const callbackEx = async (req, res, routerAuth) => {
    ...
    const { companyId, domainId, userId, accountId, connectionId, csrfToken, origin, } = customData;
    ...
    await reqidService.deleteData({}, AUTH_PATH, csrfToken);
    try {
        ...
        if (!connection) {
            console.error("Google Auth Error:Connection has not been found.", connectionId);
            return res.redirect("/login?auth_error=Google Auth Error:Connection has not been found.");
        }
        ...
        const updatedAccount = await updateTokenToServer(companyId, domainId, userId, account, token_data);
        const payload = {
            accountId,
            origin,
            mode: "view",
