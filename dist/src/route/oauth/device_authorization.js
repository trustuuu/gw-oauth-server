...
export async function device_authorization(req, res, routerAuth) {
    ...
    let clientScopes = await applicationService.getApplicationPermissionScopes(APP_PATH, client_id);
    ...
    const user_code = generateUserCode(process.env.USER_CODE_FORMAT.replace(/[A-Z0-9]/g, "A"));
    const data = {
