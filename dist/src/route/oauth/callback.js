...
async function callback(req, res, routerAuth) {
    try {
        const { companyId, domainId, email, accessToken, refreshToken, idToken, userId, } = req.body;
        ...
        res.cookie("user", JSON.stringify(user), {
            httpOnly: true,
            secure: process.env.COOKIE_SECURE === "true", // convert custom logic if needed, usually boolean
            sameSite: "lax",
            path: "/",
