 ...
 */
export const tokenExchangeGrantDownstream = async (req, res, client, targetAudience) => {
    try {
        const { subject_token, actor_token, actor_token_type, scope: scopeStr, } = req.body;
        // 3. Load Token Exchange Policy for this client
        const policies = (await applicationService.getApplicationTokenExchanges(APP_PATH, client.id)) || null;
        if (!policies) {
        ...
        const { allowedSubjectAudiences = [], allowedTargetAudiences = [], allowedScopes = [], maxTokenLifetimeSeconds = 3600, allowImpersonation = false, allowDelegation = false, } = policy;
        ...
        try {
            ...
            return res.status(400).json({
                error: "invalid_grant",
                error_description: "subject_token invalid or signature verification failed",
            });
        }
        // 5. Check subject_token audience vs policy
        if (allowedSubjectAudiences.length > 0 &&
            !allowedSubjectAudiences.includes(subjectPayload.aud)) {
            console.log("subject_token aud not allowed:", subjectPayload.aud, "allowed:", allowedSubjectAudiences);
            return res.status(400).json({
        ...
        if (allowedTargetAudiences.length > 0 &&
            ...
            console.log("target audience not allowed:", targetAudience, "allowed:", allowedTargetAudiences);
            return res.status(400).json({
                error: "invalid_target",
        ...
        if (!validateScopes(requestedScopes, allowedScopes)) {
            console.log("Requested scopes not allowed:", requestedScopes, "allowed:", allowedScopes);
            return res.status(400).json({
                error: "invalid_scope",
        ...
        const lifetimeByPolicy = (maxTokenLifetimeSeconds === "" ? 3600 : maxTokenLifetimeSeconds) || 3600;
        // Type definition says string | number | "". Handling checks.
        const expiresInSeconds = Math.min(remainingFromSubject, Number(lifetimeByPolicy));
        ...
        if (actor_token) {
            ...
            try {
                ...
                return res.status(400).json({
                    error: "invalid_grant",
                    error_description: "actor_token invalid or signature verification failed",
                });
            }
        }
...
function parseScopes(scopeStr, fallback = []) {
    if (!scopeStr)
        return fallback;
