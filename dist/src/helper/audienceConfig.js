...
export function resolveAllowedAudienceAndScopes({ policy, requestedAudience, requestedScopes, }) {
    if (!requestedAudience) {
        if (!policy.allowedTargetAudiences.length) {
            ...
            const err = new Error("No audience requested and no default audience in policy");
    ...
    if (requestedAudience) {
        const allowedAudience = policy.allowedTargetAudiences.includes(requestedAudience);
        if (!allowedAudience) {
            const err = new Error("Requested audience is not allowed for this client");
            err.oauthError = "invalid_target";
            throw err;
        }
    }
    ...
    if (requestedScopes?.length) {
        finalScopes = requestedScopes.filter((s) => policy.allowedScopes.includes(s));
    ...
    const expiresInSeconds = staticCfg.maxLifetimeSeconds ||
        policy.maxTokenLifetimeSeconds ||
        3600;
