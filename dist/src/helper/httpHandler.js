...
export function httpRun(response, fn, success, errorCallback, data) {
    return fn()
...
export function createPostHandler(serviceFn, serviceArgsFn, options = {}) {
    ...
    return (req, res) => {
        ...
        const saveOne = (record) => {
            return serviceFn.setData.apply(serviceFn, [record].concat([...serviceArgsFn(req), record.id]) //[appPath, parentId, resourceName, record.id])
            );
        };
...
export function httpPostHandler(serviceFn, buildDataFn, serviceArgsFn) {
    return (req, res) => {
        ...
        return httpRun(res, () => serviceFn.apply(applicationService, [
            data,
            ...serviceArgsFn(req),
            data.id,
        ]), null, null, data);
...
export function httpDeleteHandler(serviceFn, serviceArgsFn) {
    return (req, res) => {
        const body = req.body;
        if (Array.isArray(body)) {
            const ops = body.map((item) => serviceFn.apply(applicationService, [...serviceArgsFn(req), item.id]));
            return httpRun(res, () => Promise.all(ops));
        }
        return httpRun(res, () => serviceFn.apply(applicationService, [
            ...serviceArgsFn(req),
