export const parseScimFilter = (filter) => {
  ...
  for (const part of parts) {
    ...
    if (match) {
      ...
      // Handle numbers if not quoted
      if (!isNaN(Number(value)) && !valueRaw.includes('"')) {
        value = Number(value);
      }
...
export const applyProjection = (data, attributes, excludedAttributes) => {
  if (!attributes && !excludedAttributes) {
    return data;
  }
  let includedKeys = null;
