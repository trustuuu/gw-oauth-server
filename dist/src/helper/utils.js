...
export const generateRefreshAccessToken = async (aud, exp, client, deviceId, user, scope, token_use) => {
    ...
    const refresh_token = KJUR.jws.JWS.sign(header.alg, JSON.stringify(header), JSON.stringify(payload), privateKey);
...
export const generateAccessTokenCommon = async (selectedRasKey, iss, sub, aud, iat, exp, client, api, user) => {
    ...
    if (api.addPermissionAccessToken && user) {
        const userPermissionRaw = await userService.getUserPermissionScopes(user.companyId, user.domainId, user.id);
    ...
    if (api.RBAC && user) {
        const appRoles = await userService.getUserAppRoles(user.companyId, user.domainId, user.id, ["api", "==", api.id]);
    ...
    var access_token = KJUR.jws.JWS.sign(header.alg, JSON.stringify(header), JSON.stringify(payload), privateKey);
    return access_token;
};
...
export const generateIdTokenCommon = (iss, iat, user, clientId, scopes, nonce) => {
    ...
    var id_token = KJUR.jws.JWS.sign(header.alg, JSON.stringify(header), JSON.stringify(payload), privateKey);
    return id_token;
};
export function getTokenExchangeAccessToken({ iss, sub, aud, scopes, act, extraClaims = {}, expiresInSeconds }) {
    ...
    var privateKey = KEYUTIL.getKey(rsaKey);
    var header = { 'typ': 'JWT', 'alg': rsaKey.alg, 'kid': rsaKey.kid };
    var token = KJUR.jws.JWS.sign(header.alg, JSON.stringify(header), JSON.stringify(payload), privateKey);
    return token;
}
export const getJwtExpire = (token) => {
    ...
    if (parts.length !== 3)
        throw new Error("malform JWT");
    const payloadBase64 = parts[1];
    const payloadJson = Buffer.from(payloadBase64, "base64url").toString("utf-8");
    const payload = JSON.parse(payloadJson);
    if (!payload.exp)
        throw new Error("No expire info.");
