import { findConnection, findExternalAccount, refreshGoogleAccessToken, } from "./externalAuth.js";
// Note: 'update' from ramda was unused in original code?
// Original code: import { update } from "ramda"; ... but 'update' never used.
// Removing it.
export const tokenExchangeGrantGoogle = async (req, res, client, targetAudience) => {
    try {
        ...
        const updatedAccount = await ensureGoogleAccessToken(companyId, domainId, userId, extAccount, connection);
        const response = {
 ...
 */
async function ensureGoogleAccessToken(companyId, domainId, userId, extAccount, connection) {
    if (isAccessTokenValid(extAccount)) {
        return { ...extAccount, source: "store" };
    }
    const refreshed = await refreshGoogleAccessToken(companyId, domainId, userId, extAccount, connection);
