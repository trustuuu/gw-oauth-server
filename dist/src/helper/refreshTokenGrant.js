...
import { generateCodeAccessToken, generateIdToken, generateRefreshAccessToken, getUTCNow, } from "./utils.js";
...
export const refreshTokenGrant = async (req, res) => {
    ...
    const { client_id: clientId, refresh_token: refreshTokenBody, grant_type: grantType, } = req.body;
    ...
    if (refreshTokenBody.token_use === "refresh-device") {
        ...
        // Note: refreshTokenBodyPlain might be undefined if verification failed?
        // The original code assumes it returns something with device_id.
        if (deviceId !== refreshTokenBodyPlain.device_id) {
            console.log(`device_mismatch ${deviceId}:${refreshTokenBodyPlain.device_id}`);
            return res.status(401).json({ error: "device_mismatch" });
        }
    }
    const refreshTokenServer = await refreshTokenService.getData(AUTH_PATH, deviceId);
    if (refreshTokenServer) {
        ...
        if (refreshTokenServer.client_id != clientId) {
            ...
            // );
            console.log(`clientId_mismatch ${refreshTokenServer.client_id}:${clientId}`);
        ...
        if (!R.includes(grantType, client.grant_types)) {
            console.log("Mismatched grant_type %s does not exist in %s", grantType, client.grant_types.join(","));
        ...
        if (api.length < 1) {
            console.log("Authence has not been found, expected %s got %s", clientId, client.audience);
            return res.status(400).json({ error: "invalid_grant" });
        }
        const now_utc = getUTCNow();
        const expires_in = Math.floor(now_utc / 1000) + api[0].tokenExpiration * 60;
        const access_token = await generateCodeAccessToken(api[0].issuer, //process.env.OAUTH_ISSUER,
        refreshTokenServer.user.id, //client.client_name,
        api[0].audience, Math.floor(now_utc / 1000), expires_in, client, api[0], refreshTokenServer.user);
        const refresh_expires_in = Math.floor(now_utc / 1000) + api[0].tokenExpiration * 600;
        const refresh_token = await generateRefreshAccessToken(api[0].audience, refresh_expires_in, client, deviceId, refreshTokenServer.user, refreshTokenServer.scope, "refresh");
        const tokenClient = {
            client_id: clientId,
        ...
        if (refreshTokenServer.scope.includes("openId")) {
            ...
            Math.floor(now_utc / 1000), refreshTokenServer.user, clientId, refreshTokenServer.scope, "nonce" // Missing nonce, adding placeholder
            );
