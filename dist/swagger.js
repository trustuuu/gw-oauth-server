...
import fs from "fs";
import { DOMAIN_COLL, COMPANY_COLL, AUTH_COLL, API_COLL, APPLICATION_COLL, GROUP_COLL, USER_COLL, PROVISIONING_COLL, CONNECTION_COLL, API_PATH, APP_PATH, } from "./src/service/remote-path-service.js";
...
swaggerAutogen({ openapi: "3.0.0" })(outputFile, endpointsFiles, doc).then(async () => {
    ...
    try {
            ...
            Object.keys(swaggerObj.paths).forEach((pathKey) => {
                ...
                Object.keys(methods).forEach((method) => {
                    if (methods[method].parameters) {
                        methods[method].parameters = methods[method].parameters.filter((param) => {
                            // Remove if name is a COLL constant or redundant auth header
                            return (!constantNames.includes(param.name) &&
        ...
        swaggerString = swaggerString.replaceAll("${PermissionScopes}", "PermissionScopes");
        swaggerString = swaggerString.replaceAll("${TokenExchanges}", "TokenExchanges");
        swaggerString = swaggerString.replaceAll("${AppRoles}", "AppRoles");
        swaggerString = swaggerString.replaceAll("${UsersAndGroups}", "UsersAndGroups");
        swaggerString = swaggerString.replaceAll("${memberPath}", "members");
        swaggerString = swaggerString.replaceAll("${APPLICATION_COLL}", APPLICATION_COLL);
        swaggerString = swaggerString.replaceAll("${PROVISIONING_COLL}", PROVISIONING_COLL);
        swaggerString = swaggerString.replaceAll("${CONNECTION_COLL}", CONNECTION_COLL);
        swaggerString = swaggerString.replaceAll("${API_PATH}", API_PATH);
        swaggerString = swaggerString.replaceAll("/oauth/v1/authorize", "oauth/v1");
