...
try {
  ...
  app.use(
    session({
      ...
      cookie: {
        httpOnly: true,
        secure: process.env.COOKIE_SECURE as any,
        sameSite: "lax",
  ...
  const fetchOriginFromDB = async (origin: any, req: any) => {
    try {
      let clientId = "";
      ...
      if (
        allowedOrigins &&
        allowedOrigins.some((o: any) => o === origin || o === "*")
      ) {
      ...
      if (client.allowed_web_orgins.some((o: any) => o === origin)) {
        console.log(
          client.client_id,
  ...
  app.use("/oauth/v1", cors(corsOptions as any), authenticateOptional, routerAuth);
...
const server = app.listen((port as any) || 8080, hostheader, () => {
